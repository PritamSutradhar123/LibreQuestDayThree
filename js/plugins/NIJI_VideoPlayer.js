...
/*:
 * @target MZ
 * @plugindesc v1.3.1 Play videos wherever you want
 * @author Niji
 *
 ...
 * 
 *   Niji.Video.load(id, filename, config) - Load video into memory
...
(function() {
    ...
    PluginManager.registerCommand(pluginName, "volume", args => {
        Video.volume(args.id, Number(args.volume));
    });

    PluginManager.registerCommand(pluginName, "move", args => {
        Video.move(args.id, Number(args.x), Number(args.y));
    });

    PluginManager.registerCommand(pluginName, "scale", args => {
        Video.scale(args.id, Number(args.x), Number(args.y));
    });
    ...
    class Video {
        static load (id, name, { 
            ...
            video.__promises = [];
            video.__animations = [];
            video._texture.baseTexture.on('error', () => {
                console.error(pluginName, `[load]`, `Failed to load video "${name}". Check that the file exists in your movies/ folder, the filename is correct (including extension), and the format is .webm or .mp4.`);
            });
            ...
            video._texture.baseTexture.on('loaded', () => {
                ...
                video._source.volume = Math.min(Math.max(volume, 0), 100) / 100;
        ...
        static setAutoUnload (id) {
            ...
            if (video.__autoUnloadSet) return;
            video.__autoUnloadSet = true;
        ...
        static volume (id, v) {
            ...
            video._source.volume = Math.min(Math.max(v, 0), 100) / 100;
        }
        ...
        static play (id, shouldAutoUnload = false) {
            ...
            // Only skip if we're already visible on the video layer. The HTML element can be
            // unpaused off-stage (e.g. PIXI decoding frames after load), which must not block play.
            const spriteset = SceneManager._scene && SceneManager._scene._spriteset;
            const onVideoLayer = spriteset && spriteset._videosContainer &&
                video.parent === spriteset._videosContainer;
            if (video._source && !video._source.paused && onVideoLayer) {
        ...
        opacityAnim (alpha, duration) { Video.opacityAnim(this.video._id, alpha, duration); return this; }
